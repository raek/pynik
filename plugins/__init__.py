__all__ = ['plugins', 'command_catcher', 'first_plugin', 'auto_join', 'aduno',
            'title_reader', "pi", 'commands', 'standard', 'favorites',
            'reloader', 'tv', 'nextep', 'options', 'qotd', 'svn', 'utility',
            'example_command', 'rss', 'googlefight', 'teewars', 'tw_nazi',
            'give', 'reminder', 'kolli', 'imdb', 'mat', 'timezone',
            'ical_parser', 'icq', 'pylisp', 'lithcourse', 'scale', 'postnr',
            'tenta', 'prisjakt', 'spotify', 'stava', 'stock', 'down', 'metacritic',
            'notes', 'fml', 'systembolaget', 'randombuy', 'festern_bbq',
            'compliment', 'roulette', 'tyda', 'yrno', 'tweet', 'calc', 'degoogle', 'morse']
